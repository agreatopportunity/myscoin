...
#if defined(NDEBUG)
# error "Myscoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xdf03f589a0aad7b81868f6ff9689caa78e0d352e472a00a6468d3eeaa6e76112");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Myscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Myscoin Signed Message:\n";
...
{
    // Myscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 906 * COIN;

    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 840000); // Myscoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 24 * 60 * 60; // Myscoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Myscoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Myscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Myscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5f285df92ecfedc5e5e7122d6fb29792aa0b7bc9bfb80601eec6a11ce229c13a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "MO Vision 24/Feb/2018 Hyundai Recalls 110000";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 906 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a70cf46000d1e914084930c9e4aa2e0029d3bdde2a4623773d6bfbfae3c3d78aaa21139f157194b56f86648dbd3c9b425e4f9b502925225a033e0b3953494b2f") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1519545909;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1519767399;

        if (fTestNet)
        {
            block.nTime    = 1519545877;
            block.nNonce   = 385301817;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x6c2800df8099d6a7269d019fbff48e78dce7103092fb2eb022844728ca1bca67"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Myscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MyscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MyscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MyscoinMiner(CWallet *pwallet)
{
    printf("MyscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("myscoin-miner");
    ...
    try { loop {
        ...
        printf("Running MyscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MyscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MyscoinMiner, pwallet));
}
